...
public class CommandSt {
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		dispatcher.register(Commands.literal("st")
			.requires((iCommandSender) -> iCommandSender.getEntity() instanceof Player && (!ConfigHandler.onlyAllowCommandWhenCheatsEnabled || iCommandSender.permissions().hasPermission(Permissions.COMMANDS_ADMIN)))
			.executes((command) -> {
	...
	public static void processSilencestick(CommandContext<CommandSourceStack> command) {
		...
		silencestick.set(DataComponents.CUSTOM_NAME, Component.literal(ChatFormatting.GOLD + "The Silence Stick"));
