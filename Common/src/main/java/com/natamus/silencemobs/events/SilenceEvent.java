...
public class SilenceEvent {
	public static boolean onEntityDamage(Level world, Entity entity, DamageSource damageSource, float damageAmount) {
		...
		if (entity.isSilent()) {
			...
			if (entity.entityTags().contains(Reference.MOD_ID + ".silenced")) {
				...
				entity.entityTags().removeIf(tag -> tag.equals(Reference.MOD_ID + ".silenced") || tag.startsWith(Reference.MOD_ID + ".name."));
